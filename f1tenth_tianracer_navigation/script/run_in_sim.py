...
def middle_line_callback(data):
    ...
    speed = 1.0*(0.3*math.exp(-np.clip(abs(angle),0,0.5))+0.7)
    #absangle = abs(angle)
    angleabs = abs(angle)
    ...
    # speed = -435.8974*pow(angleabs,5)+481.9347*pow(angleabs,4)-142.2789*pow(angleabs,3)-1.0184*pow(angleabs,2)-0.0233*angleabs+3.1926
    ...
    if max_dir_index!=0 and len(Left_obs) ==0:
      ...
      if mean_max_dis > MAX_DETECT_THRESHOLD and mean_max_dis < 5.5:
        incre = mean_max_dis / MAX_DETECT_THRESHOLD
        speed = (0.0910*pow(incre,2)-0.0604*pow(incre,1)+0.9697)*speed
      elif mean_max_dis >= 2.5   :#前面8米没障碍
        speed = speed*1.1
