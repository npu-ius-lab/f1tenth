...
def middle_line_callback(data):
    ...
    #改障碍，小就去除
    Left_obs_temp_2 = list()
    for i in range(len(Left_obs)/2):
      if Left_obs[i+1] - Left_obs[i] > 8:
        Left_obs_temp_2.append(Left_obs[i])
        Left_obs_temp_2.append(Left_obs[i+1])
    Left_obs = list()
    Left_obs = Left_obs_temp_2
    ...
    print("steering angle is ",steering_angle*180/3.14)
    '''if angle > 0:
    ...
    if max_dir_index!=0 and len(Left_obs) ==0:
      ...
      if mean_max_dis > MAX_DETECT_THRESHOLD and mean_max_dis < 5.5:
        incre = abs(mean_max_dis / MAX_DETECT_THRESHOLD)
        speed = (0.0910*pow(incre,2)-0.0604*pow(incre,1)+0.9697)*speed
      elif mean_max_dis >= 5.5 :#前面8米没东西
        speed = speed*1.1
    print(speed)
